      ...
      <div class="main-controls">
        ...
        <div class="left-controls">
          <button @click="playPrevious()" class="sketch-icon" title="上一首" :disabled="!player.currentSong">
            <svg width="20" height="20" viewBox="0 0 16 16" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" fill="none">
          ...
          <button @click="player.togglePlay()" class="sketch-icon sketch-icon-play" title="播放/暂停" :disabled="!player.currentSong">
            <svg v-if="player.loading" width="26" height="26" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.2" stroke-linecap="round">
              <circle cx="8" cy="8" r="6" stroke-dasharray="3,2"/>
            </svg>
            <svg v-else-if="player.isPlaying" width="26" height="26" viewBox="0 0 16 16" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" fill="none">
              <line x1="4.5" y1="3" x2="4.5" y2="13"/><line x1="11.5" y1="3" x2="11.5" y2="13"/>
            </svg>
            <svg v-else width="26" height="26" viewBox="0 0 16 16" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" fill="none">
        ...
        <div class="right-controls">
          ...
          <div class="volume-controls">
            <button @click="player.toggleMute()" class="sketch-icon" title="音量" style="padding: 2px;">
              <svg v-if="!player.muted && player.volume > 0" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round">
          ...
          <button
            ...
            style="padding: 2px;"
          >
            <svg v-if="player.currentSong && player.isFavorite(player.currentSong)" width="16" height="16" viewBox="0 0 24 24" fill="currentColor" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="fav-on">
          ...
          <button @click="player.togglePlayMode()" class="sketch-icon" :title="player.playModeLabel" style="padding: 2px;" :disabled="!player.currentSong">
            <svg v-if="player.playMode === 'random'" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round">
            ...
            <svg v-else width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round">
              ...
              <text
                v-if="player.playMode === 'single' || player.playMode === 'loop'"
                x="8"
                y="15"
                font-size="6"
                fill="currentColor"
...
const shouldMarquee = computed(() => (player.currentSong?.title?.length || 0) > 12);
...
function seekFromClientX(clientX: number) {
  ...
  const pct = progressFromClientX(clientX) / 100;
...
function onPointerDown(e: PointerEvent) {
  ...
  dragProgress.value = progressFromClientX(e.clientX);
  el.setPointerCapture(e.pointerId);

  const onMove = (ev: PointerEvent) => {
    dragProgress.value = progressFromClientX(ev.clientX);
  };
